...
export function compareFacts(site, poolId, facts, today = new Date()) {
  ...
  for (const s of facts.specialSchedules || []) {
    ...
    for (const d of eachDay(s.from, s.to)) {
      ...
      if (site.isClosedDay(pool, d)) { closedButSpecial.push(d); continue; }
...
export async function watchNotices({ site, log = console.log, today = new Date() } = {}) {
  ...
  for (const p of posts) {
    ...
    if (old && !edited && !retryNoKey) continue;
    ...
    const items = compareFacts(site, p.poolId, r.facts, today);
    if (items.length) fresh.push({ ...entry, summary: r.facts.summary, items });
  ...
  for (const e of Object.values(state.posts)) {
    ...
    const items = compareFacts(site, e.poolId, e.facts, today);
